...
跑完 5 篇（每篇都走完 `/oracle-predict` → 发布 → `/oracle-retro` 闭环）之后，你会有第一份个人校准数据，可以提议第一次 `/oracle-bump` 升级到 v1（或直接采用 [opinion-video.md](opinion-video.md) 的 v2 作为起点重新校准权重）。
...
跑完 5 篇后，你的 baseline 已经从 5 个数据点稳定。`/oracle-status` 会主动提示：

> 你已校准 5 篇，可以基于实际分布固定绝对 bucket 边界。
> 跑 `/oracle-bump --bucket-only` 自动派生。
...
- 第 5 篇之后**必须**跑 `/oracle-bump --bucket-only` 重新校准
...
### 路径 A：从 v0 → v1（自己拟合权重）
跑 `/oracle-bump --propose "<你的具体权重调整>"`。系统会强制全量重打 + 跨模型审核。

### 路径 B：直接采纳 v2 作为起点
把 [opinion-video.md](opinion-video.md) 的 v2 公式抄到你的 `rubric_notes.md`，**然后跑 `/oracle-bump`**——bump 流程会用你的 5 篇数据验证 v2 在你账号上是否真的更优。
