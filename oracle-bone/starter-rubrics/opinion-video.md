...
> **如果你想试 v2.1**：跑完 5 篇 v2 校准 + 主动收集 MS 和 TS 的"试评分"作为候选维度数据。证据足够（≥3 样本支持）后通过 `/oracle-bump` 走完整升级流程。
...
> Calibration 用户（≥5 篇有数据）由 `/oracle-init` 询问 baseline 后自动派生。
...
校准过 ≥5 篇后跑 `/oracle-bump --bucket-only` 让系统基于你的实际分布自动派生。
