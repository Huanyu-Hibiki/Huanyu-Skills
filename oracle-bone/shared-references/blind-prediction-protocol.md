...
被这些子 skill 引用：`oracle-predict`、`oracle-retro`、主 SKILL.md。
...
子 skill 在启动 `oracle-predict` 前，必须自检并向用户**主动声明**：

1. **作品已发布超过该轨道第一个 retro 窗口**（默认 3 天）→ 必须拒绝写"预测"，改记为 `**Reconstructed retrospective**`，明确标注非预测
2. **作品已发布但 < retro 窗口，用户尚未透露任何数据** → 允许 blind 预测，但在文件头部标记 `published_before_prediction: true` + `blind_status: confirmed_no_data_seen`
...
唯一允许编辑预测段的场景：**纯 markdown 排版错误**（标题层级错误、列表 bullet 格式错），且用户明确说明这是格式修复。这种情况 hook 仍会阻塞，需要用户显式 bypass（手动设置环境变量 `ORACLE_BYPASS_IMMUTABILITY=1` 单次）——bypass 应在 git history 留痕。

---

## 文件名约定（**两处一致**）

一个内容两处文件，**用同一组 `<date>_<id>_<short>` 命名**（都在作品目录内）：

```
<NNN>_<标题>/scripts/<date>_<id>_<short>.md        ← pre-production 草稿（oracle-seed 写或用户写）
<NNN>_<标题>/predictions/<date>_<id>_<short>.md    ← immutable 预测（oracle-predict 写，复盘段追加于此）
```
...
Reconstructed 重做：在 `<short>` 后加 `_redo`，两处都加。原文件保留（不删）。
...
`oracle-predict` 启动时：
1. 读 `BLIND_CHECK` 常量
2. 询问用户该作品当前发布状态（未发 / 已发 < retro 窗口 / 已发 ≥ retro 窗口）
...
| 场景 | 处理 |
|---|---|
| 预测文件不小心被人手编辑了预测段 | 不自动回滚（破坏更大）。下次 `oracle-retro` 检测到不一致 → 在复盘段追加 `**Integrity warning**: 预测段于 <ISO timestamp> 被外部修改，无法保证盲度`，校准价值降级为"参考"，不计入 bump 校准池 |
...
盲预测是整个 oracle-bone 校准循环的**唯一信号源**。一旦预测段被事后修改，所有"哪个维度被验证 / 推翻"的判断都失去基线——你不知道当初是真预测对了，还是事后改对了。
