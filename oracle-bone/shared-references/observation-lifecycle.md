...
被这些子 skill 引用：`oracle-retro`、`oracle-bump`、主 SKILL.md。

这是项目原则 #3 的完整规范——**rubric 是工作台，不是博物馆**。任何对 `rubric_notes.md` 中观察条目的增删都必须遵循本协议。观察按轨道分节管理，互不污染。
...
|---|---|---|
| **观察记录** | 该轨道 `## 观察记录` 段 | 单次复盘后写入。每篇作品复盘对应一条 |
| **跨样本观察** | 该轨道 `## 重大跨样本观察` 段 | 同一个 pattern 在 ≥2 个样本里出现 |
| **规律沉淀** | 该轨道 `## 规律沉淀区（高置信度）` 段 | 已有 ≥2 样本支持且**通过升级验证流程**（即被吸收进维度或被显式确认） |
| **待验证假设** | 该轨道 `## 待验证假设` 段 | 单样本观察暂存，等更多样本 |
...
|---|---|---|
| 观察记录 → 跨样本观察 | 同 pattern ≥2 样本 | 1 样本 + 极强模因证据（如单条评论 ≥2000 赞）也可升 |
| 跨样本观察 → 待验证假设 | 单样本 + 强信号但还没复现 | — |
| 跨样本观察 → 规律沉淀 | ≥2 样本 + 不需要改公式 | 1 样本 + 强反例（≥3x 偏差）也可升（标 `**Single-sample, high-confidence**`） |
| 跨样本观察 → 维度（不再单独存在） | ≥3 样本 + 通过 bump 流程被吸收 | 同 bump 协议的触发启发式 |
| 任意 → 删除 | 被新数据推翻 / 被吸收为维度 / 被沉淀为规律 | `oracle-bump` cleanup pass，或单独操作 |

**核心原则**：样本数是**信号强度的代理指标**，不是信号本身。3 个清晰、可追溯到具体数据点的样本 > 10 个零碎、低置信度的样本。

**Claude 软违反的纪律**：
- 标注 `**Promoted with N samples (default expects M)**: <为什么仍然成立>`，让用户审视
- 不连环软违反——如果最近 3 次升格里有 2 次以上软违反，oracle-status 提示"你的观察升格判断可能太激进，建议回到默认门槛 review"

### Sample size → 允许的动作（细化分级）

按该轨道校准池**总样本数**决定能做什么改动——粗暴改大动作需要更多证据：

| 校准池规模 | 允许的动作 | 备注 |
|---|---|---|
| **1 个** | 记录"单次观察" | 单点不能触发任何规则改动，只能作为种子 |
| **2-4 个** | 提炼"候选规律" + 升级到"跨样本观察"段 | 仍不能改公式 |
...
例：升级时，"观察 E（致谢段二创量级是 ER=5 的最强外部证据）"被吸收为新维度 **MS（Memetic Shareability）**。
...
例：观察 X 提议"长内容天花板低"，后来 4 个长内容样本都破 50w → 推翻。
...
- **历史校准事件**（如"v1 → v2 升级是因为某样本严重低估"）→ 保留在版本日志的"升级 Memo"段，**不**保留在观察段
...
- `oracle-status` 看板检测到 `rubric_notes.md` 行数超过 500 时，建议清算
...
- Top 评论关键词：[简短摘录 + 赞数]
- 判断：哪个维度被验证 / 推翻？为什么？
- Rubric 调整：[如果有，写明 "下次打 XX 类内容时改 YY"]
...
`oracle-bump` 在 Step 5（cleanup pass）必须按以下顺序处理该轨道观察段：
