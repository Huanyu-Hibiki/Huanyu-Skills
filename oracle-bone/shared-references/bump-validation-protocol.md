...
被这些子 skill 引用：`oracle-bump`、主 SKILL.md。

这是项目原则 #2 的完整规范——**升级 = 全量重打**。任何对 rubric 的结构性变更都必须走完本协议。**按轨道独立执行**：每个轨道有自己的 rubric 节和校准池分桶，bump 一次只处理一个轨道。
...
下列变更**不**触发 bump，但要在 `rubric_notes.md` 对应轨道节标注：
...
1. 某个跨样本观察出现**清晰、可追溯到具体数据点的支持模式**（默认参考 ≥3 样本，但 Claude 可以基于 1-2 个**特别强**的样本提议——如 6x 流量比这种异常信号）
...
4. 该轨道校准池跨过某分水岭（5 / 10 / 20 / 50）触发常规复审
...
- 上次 bump 后该轨道没有任何新校准样本——必须至少 1 个新样本才能再次 bump（避免循环验证自己）

下面是**软建议**——Claude 通常应避免，但有强理由可以打破并显式标注：

- 该轨道校准池 < 5 样本时一般不 bump（除非出现强反例）
...
不能只说"ER 权重提高"，必须把整个方程完整写出，所有系数和归一化常数，并标注**作用于哪个轨道**。

示例（合规）：
```
[track: reach] v2.0  composite = (ER×1.5 + SR×1.5 + HP×1.5 + QL + NA + AB + SAT) / 8.5 × 2.0
[track: reach] v2.1  composite = (ER×2.0 + HP×1.5 + MS×1.5 + QL + SR + TS + SAT) / 9.0 × 2.0
```
...
### Step 2：在该轨道校准池上**本地**全量重打分

校准池定义：该轨道所有有完整复盘段（含 `actual_*` 数据）的 prediction 文件（`state.calibration_samples.by_track[<track>]` 登记的样本）。
...
2. 用实绩指标（按该轨道 success_metrics 选主指标）给所有样本排序
...
1. 调用外部 LLM（跨模型审核通道——MCP 工具或用户配置的第二模型），把以下打包发过去：
   - 轨道标识 + 旧公式 + 新公式
   - 该轨道校准池所有样本的：维度分数、composite（新+旧）、实绩数据
   - Step 3 的排序对照表
2. 外部 LLM 独立判定两件事：
   - 排序一致性是否真的 ≥ THRESHOLD？
   - 新公式相比旧公式是否解释力更强？
...
`CROSS_MODEL_AUDIT=false`：跳过外部审核——**仅在离线/无审核通道时使用**。state file 标记 `last_bump_self_audited: true`，`oracle-status` 持续提示用户配置外部审核。

### Step 5：升级后清算（Cleanup pass）

bump 落地必须一次性完成：

1. 在 `rubric_notes.md` 该轨道节顶部更新 `**当前版本**` 和**版本速查表**新增一行
2. 在该轨道的"vN → vN+1 升级 Memo" 段写完整 memo（触发观察 + 证据数据 + 诊断 + 新公式 + 已知局限）
