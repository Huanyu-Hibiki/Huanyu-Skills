...
被 `oracle-migrate` skill / `oracle-init` / SessionStart hook / 维护者引用。规定如何安全演进 `.oracle-state.json` schema 而不让老用户被打断。
...
- **改字段语义**（如 `baseline_plays` 从 int 改为 list）
...
### 1. 改 oracle-init 写新 state 的硬编码 schema_version
...
位置：`skills/oracle-init/SKILL.md` Phase 5 的 state 写入段。
...
- WHAT changed
- WHY
- HOW (AI steps for /oracle-migrate)
- Manual fallback
...
**例外**：核心标识字段允许直接索引（如 `state["schema_version"]`）——这些缺失意味着 state 文件根本不合法，应该明确报错。
...
if [[ "$state_schema" != "$LATEST_SCHEMA" ]]; then
  ...
  echo "   建议跑 /oracle-migrate 升级（不阻塞继续工作）"
fi
```
...
`/oracle-migrate` 写之前会备份到 `.oracle-state.json.backup-<timestamp>`。

备份保留多久：
- 用户跑 `/oracle-status` 时，如果有备份 + state 已稳定运行 N 天 → 提示"可以清理 N 个旧备份"
- `/oracle-init` 重 init 时清理所有旧备份（既然要重 init，老备份意义不大）
- 用户手动 `rm .oracle-state.json.backup-*` 永远 OK

不入版本控制：`.oracle-state.json.backup-*` 应在 `.gitignore` 里（已含 `.oracle-state.json` 通配规则）。
