...
被这些子 skill 引用：`oracle-trends`、`oracle-recommend`、`oracle-init`、所有 `adapters/`。
...
| `track` | enum / null | **轨道 id**——取值来自 `.oracle-state.json` 的 `tracks.definitions[].id`（init 内容规划的轨道，如 reach / cognition / convert / 用户自定义）。交叉题写 `cross:<t1>+<t2>`。`null` = 未分流（oracle-seed 分流时必询问后写入）。oracle-bump 升级时按轨道分桶重打，不互相污染 |
| `read_status` | enum | `unread` / `skimmed` / `deep_read` / `done`。处理状态 |
| `category` | string | 自定义分类标签（如"社会"、"家庭"、"行业"） |
| `composite_score` | float | 用当前 rubric（该轨道节）打分得到的综合分（如已打） |
...
打分前 score 字段全部为 null——是预期的。`oracle-trends` 抓回来后会调 `oracle-score` 给每条算 composite。
...
升级到 SQLite 之后（见 `oracle-status` 的升级触发），同样字段走 `articles` 表存储，markdown 视图自动从 DB 渲染。
...
`oracle-trends` / `oracle-recommend` 在写入 `candidates.md` 前必须执行：

1. 计算新 item 的 id
2. 检查 `candidates.md` 是否已含此 id → 跳过
3. 检查各作品 `predictions/*.md` 是否含此 id（已发过）→ 跳过
4. 检查 `.oracle-cache/trends-history.jsonl` 是否含此 id 且 `rejected_at != null` → 跳过（用户已主动拒绝过）
5. 通过则写入

`.oracle-cache/trends-history.jsonl` 是抓取历史的去重缓存，每行一个 JSON record，append-only。被用户拒绝的候选会在这里保留 6 个月；之后允许重新出现（也许素材在新 rubric 下评估不同）。
...
| Tier | 含义 | 对应行动 |
|---|---|---|
| `tier1` | 强候选，应推荐 | 进入 `oracle-recommend` 排序池 |
...
**Cold-start 期间所有 item 默认是 `unread`/`null tier`**——直到用户或 `oracle-score` 给出 composite 后才能粗分类。**未打分的 item 不应出现在 `oracle-recommend` 输出**——避免推荐没读过的素材。
...
`track` 字段是 oracle-bone 的**必填字段**（cold-start 期允许 null）。

| 值 | 含义 | 来源 |
|---|---|---|
| `<track-id>` | 单一轨道（init 规划注册的 id，如 reach / cognition / convert） | oracle-seed 分流时询问后写入 |
| `cross:<t1>+<t2>` | 交叉题（一条内容跨两轨） | 各轨 rubric 各打一次取均值；复盘时两轨校准池各 +0.5 |
| `null` | 未分流（历史候选） | oracle-recommend 用 state 占比兜底 |

**写入时机**：oracle-seed 分流时必询问用户后立刻写入。

**读取时机**：oracle-recommend / oracle-predict / oracle-retro / oracle-bump 全部要读，按 track 分别处理。

**与 state.tracks 的关系**：
- `tracks.definitions[].mix_ratio` 是各轨占比——oracle-recommend 按 ratio 过滤推
- 当 track=null（历史候选）——oracle-recommend 用 ratio 兜底
- 当 track 已设置——oracle-recommend 直接按 track 字段过滤

**oracle-bump 分桶**：`calibration_samples.by_track[<track>]` → 该轨 rubric 独立升级；cross 题 → 两个池各 +0.5。
