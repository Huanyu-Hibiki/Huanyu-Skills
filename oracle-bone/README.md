...
初始化不是建几个空文件。`/oracle-init` 会通过采访为用户建立完整档案：

- **你是谁**：变现方式、专业优势、形象定位（创作者档案）
...
装一次，所有内容项目都能用。各 runtime 的 skills 目录参考：
...
每次开会话 hook 自动报告 buffer + 待复盘 + top 候选——你不用主动问（hook 为 Claude Code 格式；不支持 hooks 的 runtime 用「状态」触发 oracle-status 手动补位）。

完整工作流 + 子 skill 细节见 [SKILL.md](SKILL.md)，完整设计见 [DESIGN.md](DESIGN.md)。
...
> 前身：cheat-on-content（源自抖音蜗牛学长，经实战大幅扩展）。本版剥离个人定制，
> 保留完整方法论与全部 26 个子 skill 流程，成为通用内容校准器。
