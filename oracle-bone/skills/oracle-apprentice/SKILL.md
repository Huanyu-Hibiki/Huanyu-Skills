...
| 5. 术语处理 | 专业词第一次出现怎么落地（大白话/类比/数字）；博主有个人黑话/独特用法 → 记「他的用法 vs 常识用法」差异（key_distinction），用户会引用的术语顺手提示入 voice-lexicon.md 专名表（走确认） | 落地手法 + 术语区分 |
...
输出含"值得偷的 3 样东西"（每样附"怎么偷——用到你的哪轨哪段"）→ 逐项过 Phase 3.5 验证门后才可进卡片。

### Phase 3.5: 三重验证门（「值得偷」项进卡片前的硬门——机制思想借鉴 cangjie-skill，MIT）

对每个「值得偷的 X」逐项过三门——**验证不过 = 一次性技巧，留在 analysis.md「未通过验证」节（写明卡在哪门 + 一句原因），不进知识卡片**：

| 门 | 判据 | 不过的典型 |
|---|---|---|
| **V1 跨域** | 这套手法至少在 **2 个独立语境**成立：本条 + 你能举的另一条该博主稿件 / 另一个博主 / 你自己轨道的真实案例。同一手法换个说法不算第二个语境 | 只在这条稿的特定题材上成立的一次性技巧 |
| **V2 预测力** | 用这套手法能推演出一个**没拆过的场景**的具体做法（如：用该钩子骨架推演你轨道一条新开场的起手三步）。推不出非平庸结论 = 只是"看着有道理" | 金句很帅但说不清下一条怎么用 |
...
**触发场景**：<何时想起用它：2-3 条情境 + 语言信号（写到哪段/卡在哪时会用到）——防止"学过但想不起来用">
**反例**：<失效场景 + 为什么人会掉进去 + 预警信号——三段齐才合格>
**边界条件**：<时长/赛道/人设阶段的前提>
...
| 「值得偷」项全部没过验证门 | 如实报——本条只留单条笔记不出卡片；换一条拆或降低预期（这条稿可能真没货） |
| 用户要求验证全过、全进卡片 | 拒绝降门——全过 = V3 失守（常识混进卡片）；每条要么补证据重验，要么留档不进 |
...
7. **三重验证门**——V1 跨域 / V2 预测力 / V3 独特性，不过不进卡片、留档写原因（Phase 3.5 硬约束）
...
- ❌ 验证门放水——「值得偷」项全过 = V3 失守，人人都说的常识混进卡片等于没拆
