...
| `video-caption-correct/` | 字幕人工复核工具链：审核页、词级详情、口癖裁决合并、词典沉淀辅助（legacy：独立转录入口 run_transcribe / 火山云脚本 / FCPXML 辅助仅离线场景保留） | `doctor.js`、`generate_review.js`、`extract_text.js`、`lib/` |
| `video-rough-cut/` | faster-whisper/Whisper 转录（`--lexicon` 词典偏置）、文稿自动校对（align_to_manuscript：字幕采用文稿拼写 + ASR 偏差/口癖候选落盘）、字幕映射、EDL 渲染、调色、时间线预览 | `transcribe.py`、`align_to_manuscript.py`、`render.py` |
