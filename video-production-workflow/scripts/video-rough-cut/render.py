...
Usage:
    python scripts/video-rough-cut/render.py <edl.json> -o final.mp4
    python scripts/video-rough-cut/render.py <edl.json> -o preview.mp4 --preview
    python scripts/video-rough-cut/render.py <edl.json> -o final.mp4 --build-subtitles
    python scripts/video-rough-cut/render.py <edl.json> -o final.mp4 --no-subtitles
"""
...
def resolve_path(maybe_path: str, base: Path) -> Path:
    """Resolve a path relative to the EDL directory or its project root."""
    p = Path(maybe_path)
    if p.is_absolute():
        return p.resolve()

    candidates = [base / p, base.parent / p, Path.cwd() / p]
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=int,
        default=24,
        help="Output frame rate for extracted segments. Default: 24.",
    )
    args = ap.parse_args()
