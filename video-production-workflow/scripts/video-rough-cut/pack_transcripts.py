...
Usage:
    python scripts/video-rough-cut/pack_transcripts.py --edit-dir <edit_dir>
    python scripts/video-rough-cut/pack_transcripts.py --edit-dir <edit_dir> --silence-threshold 0.5
"""
...
def pack_one_file(json_path: Path, silence_threshold: float) -> tuple[str, float, list[dict]]:
    ...
    try:
        ...
        content = json_path.read_text(encoding="gbk")
...
def main() -> None:
    ...
    json_files = sorted(p for p in transcripts_dir.glob("*.json") if p.name != "subtitles_words.json")
    if not json_files:
