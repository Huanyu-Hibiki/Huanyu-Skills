...
Usage:
    python scripts/video-rough-cut/timeline_view.py <video> <start> <end>
    python scripts/video-rough-cut/timeline_view.py <video> <start> <end> -o out.png
    python scripts/video-rough-cut/timeline_view.py <video> <start> <end> --n-frames 12
    python scripts/video-rough-cut/timeline_view.py <video> <start> <end> --transcript <path>
    python scripts/video-rough-cut/timeline_view.py --edl <edl.json>   (full-project view — not yet)
"""
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help="Path to transcript.json for word labels + silence shading. "
             "If omitted, will auto-resolve to <video_parent>/Rough/transcripts/<video_stem>.json",
    )
    ap.add_argument(
        "--edl",
        type=Path,
    ...
    if transcript is None:
        auto = video.parent / "Rough" / "transcripts" / f"{video.stem}.json"
        if not auto.exists():
