 ...
 *   - 全绿后写当前实现目录/.setup_done，后续运行可跳过引导
 */
...
const AI_SCRIPTS_DIR = path.resolve(__dirname);
const WORKFLOW_DIR = path.resolve(__dirname, '..', '..');
const ROOT_ENV = path.join(WORKFLOW_DIR, '.env');
const SENTINEL = path.join(AI_SCRIPTS_DIR, '.setup_done');
// .env 候选位置（与 agent / 安装位置无关）：显式指定 → skill 内 → skill 上一级（兼容 Claude Code 旧约定）
const ENV_CANDIDATES = [
  process.env.VOLCENGINE_ENV_FILE,
  ROOT_ENV,
].filter(Boolean);
const RECOMMEND_ENV = ROOT_ENV;
...
(async () => {
  ...
  const envOk = env.state === 'ok' || env.state === 'optional_missing';
  const resOk = env.state === 'optional_missing' || (!!res && res.flash === 'ok' && res.std === 'ok');
