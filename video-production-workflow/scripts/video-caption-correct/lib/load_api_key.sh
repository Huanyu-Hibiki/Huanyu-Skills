...
_script_dir="$(cd "$SCRIPT_DIR" && pwd)"
_workflow_dir="$(cd "$_script_dir/../.." && pwd)"
. "$_workflow_dir/scripts/lib/load_env.sh"
_venv_python=""
if [ -x "$_workflow_dir/.venv/bin/python" ]; then
  _venv_python="$_workflow_dir/.venv/bin/python"
elif [ -x "$_workflow_dir/.venv/Scripts/python.exe" ]; then
  _venv_python="$_workflow_dir/.venv/Scripts/python.exe"
fi
if [ -z "$_venv_python" ] || ! "$_venv_python" -c 'import sys' >/dev/null 2>&1; then
  echo "❌ 找不到 Skill .venv，请先在合集根目录执行：uv venv .venv --python 3.11 && uv sync" >&2
  exit 1
fi
...
if [ -z "$API_KEY" ]; then
  for _ef in "${VOLCENGINE_ENV_FILE:-}" "$_workflow_dir/.env"; do
...
if [ -z "$API_KEY" ]; then
  ...
  echo "   方式一(推荐)：在 $_workflow_dir/.env 写一行  VOLCENGINE_API_KEY=你的key" >&2
