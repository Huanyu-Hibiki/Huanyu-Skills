...
volc_python_bin() {
  _workflow_dir="$(cd "$(dirname "$SCRIPT_DIR")/.." && pwd)"
  for _python in \
    "$_workflow_dir/.venv/bin/python" \
    "$_workflow_dir/.venv/Scripts/python.exe"; do
    if [ -x "$_python" ] && "$_python" -c 'import sys' >/dev/null 2>&1; then
      printf '%s\n' "$_python"
      return 0
    fi
  done
...
volc_gen_request_id() {
  local id
  id=$(uuidgen 2>/dev/null || "$(volc_python_bin)" -c "import uuid; print(uuid.uuid4())")
  printf '%s' "$id" | tr '[:upper:]' '[:lower:]'
}

# volc_build_request <audio_file_or_url> <out_file>：写请求体 JSON 到 out_file
volc_build_request() {
  "$(volc_python_bin)" "$SCRIPT_DIR/lib/build_request.py" "$1" > "$2"
}
...
volc_word_count() {
  "$(volc_python_bin)" - "$1" <<'PY' 2>/dev/null || echo "?"
import json, sys
