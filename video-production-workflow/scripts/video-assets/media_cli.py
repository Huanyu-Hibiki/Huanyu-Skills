...
# YouTube 下载配置
YTDLP_COOKIES = os.environ.get('YTDLP_COOKIES', '')  # Cookies 文件路径
# 默认 cookies 位置（按优先级）
...
def find_executable(name: str, extra_paths: List[str] = None) -> Optional[str]:
    """查找可执行文件，支持额外路径"""
    import shutil
    # 先在 PATH 中查找
    ...
    if extra_paths:
        for base_path in extra_paths:
            ...
            if os.path.isdir(base_path):
                for exe_file in os.listdir(base_path):
                    if exe_file.lower().startswith(name.lower()) and exe_file.lower().endswith('.exe'):
                        full_path = os.path.join(base_path, exe_file)
                        return full_path
    return None
...
class ImageDownloader:
    ...
    def search_pixabay(self, query: str, count: int = 5) -> List[Dict]:
        ...
        try:
            ...
            print(f"Pixabay 图片搜索失败: HTTP {resp.status_code}")
        except Exception as e:
...
class VideoDownloader:
    ...
    def __init__(self):
        ...
        self.ytdlp_available = bool(self.ytdlp_path)
        self.ffmpeg_available = bool(self.ffmpeg_path)

    def _find_ytdlp(self) -> Optional[str]:
        """查找 yt-dlp 可执行文件"""
        path = find_executable('yt-dlp', self.extra_ytdlp_paths)
        if path:
            return path
        # 直接检查常见路径
        for base in self.extra_ytdlp_paths:
            if os.path.isdir(base):
                for f in os.listdir(base):
                    if 'yt-dlp' in f.lower() and f.lower().endswith('.exe'):
                        return os.path.join(base, f)
        return None

    def _find_ffmpeg(self) -> Optional[str]:
        """查找 ffmpeg 可执行文件"""
        return find_executable('ffmpeg', self.extra_ffmpeg_paths)
                ...
                for hit in data.get("hits", []):
                    ...
                    for quality in ["large", "medium", "small"]:
                        if quality in videos:
                            ...
                            break
                return results
            print(f"Pixabay 视频搜索失败: HTTP {resp.status_code}")
        except Exception as e:
    ...
    def download_youtube(self, url: str, output_dir: str = None,
        ...
        if self.cookies_path:
            cmd.extend(['--cookies', self.cookies_path])
            print(f"🍪 使用 Cookies: {self.cookies_path}")
...
class VideoTrimmer:
    ...
    def _find_ffmpeg(self) -> Optional[str]:
        ...
        for base in extra_paths:
            if os.path.isdir(base):
                for f in os.listdir(base):
                    if 'ffmpeg' in f.lower() and f.lower().endswith('.exe'):
                        return os.path.join(base, f)
        return None
...
def cmd_status(args):
    ...
    cookies_status = f"✅ {cookies_path}" if cookies_path else "❌ Not configured"
    if len(cookies_status) > 50:
        cookies_status = "✅ " + cookies_path[:40] + "..."
    print(f"║    Cookies:   {cookies_status:<50} ║")
    print(f"║    💡 Set YTDLP_COOKIES to a local cookies file             ║")
