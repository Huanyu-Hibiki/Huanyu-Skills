...
7. **Take 挑选（多遍重读必跑）**：运行 `select_takes.py`，找出每句文稿的所有 take 并按匹配度/完整度/停顿/语速打分选最佳，产出 `takes_decision.md` 给用户过目；未匹配句子必须逐条确认（没读 or ASR 太差）。**幻觉硬闸**：低置信词占比 >35% 的候选 take 直接拒绝（Whisper 在停顿处幻觉补尾，转录有字、音频是静音——截断句会"看起来完整"），该句进未匹配清单交人工裁决，宁缺不滥。用户裁决有歧义的 take（补读/改文稿/保留最长）后，把裁决追加进项目根 `decision_log.json`（category=`take_selection`）。
8. **卡顿/重复剪除（默认开启）**：运行 `detect_repeats.py`（输入 `finalKeeps_<stem>.json`），在保留段内检测口播不流畅——**词级结巴**（"我们我们来看"，同词连读）和**前缀废弃**（说一半停住重说完整："我觉得这个 → 我觉得这个方案"）**自动剪除首次尝试**，产出 `keeps_dedup_<source>.json`；**非前缀的相似重说**（"非常实用/非常好用"——与排比句文本上无法区分）和**长间隔整句重读**（>2s，select_takes 领域）只进 `repeats_report.md` 待确认清单，**不自动剪**，逐条请用户裁决（🔴 句级删除 diff 确认；确认要剪的手工并入 keeps 再重跑）。方向保险：删除段长于保留段的候选一律降级待确认。
...
10. **keeps 回读校验（保险丝，EDL 前必跑）**：运行 `verify_keeps.py`，把 keeps 覆盖的转录文本拼回与文稿逐句核对——**句子截断**（句尾在成片中缺失，如"……学习引擎"被剪成"……成本做"）、**重复未剪**（同句出现多次）、**疑似幻觉词**（低置信占比超阈值）三类 finding 任一存在即 fail：🔴 **fail 不得进入 EDL**，findings 逐条交用户裁决。
11. 基于 `takes_decision` + `keeps_tightened` 提出 EDL；**EDL 只能使用被选中的 take，重复 take 和被淘汰 take 不得进入草稿**；不在词中间切断。
12. 先做分段提取、音频淡入淡出和无损 concat，再按需要加入覆盖层。
13. 生成粗剪预览和 `rough_cut_manifest.md`、`missing_materials.md`。
14. 自检接缝、音频爆音、字幕预留空间和内容完整性——渲后对**成片**每个切点 ±1.5s 窗口抽帧+抽波形核对（视觉跳变/爆音/字幕被遮），不拿源素材当检查对象。
...
# keeps 回读校验（EDL 前必跑）：句子截断/重复未剪/幻觉词三查，fail 不进 EDL
uv run --project "<合集根>" python "<合集根>/scripts/video-rough-cut/verify_keeps.py" \
  "<项目>" "<项目>/Rough/transcripts/subtitles_words.json" \
  "<项目>/Rough/keeps_tightened_<stem>.json"
...
- **幻觉 take 不进草稿**：低置信词占比 >35% 的候选 take 一律拒绝（转录有字、音频静音）——截断句被 Whisper 幻觉补尾后"看起来完整"，评分无法区分，只有词级置信度能识破；
- **keeps 回读校验是 EDL 的前置保险丝**：fail（句子截断/重复未剪/幻觉词）不得生成 EDL，逐条交用户裁决；
...
| 句子因幻觉硬闸被拒（低置信 take） | 检查该句的词级置信度分布——多为 Whisper 停顿幻觉补尾，真实语音在后续重读里 | 🔴 未匹配句交用户裁决；重跑转录（换引擎/加 `--initial-prompt`）后再试，不硬选幻觉 take |
| `verify_keeps` fail（截断/重复/幻觉 finding） | 按 finding 类型路由：截断→查 take 边界重跑 select；重复→走 `detect_repeats` 待确认清单；幻觉→回转录 | 🔴 fail 不得进 EDL——findings 逐条交用户裁决后修正 keeps 再验 |
...
├── keeps_verify_report.json           # 回读校验报告（截断/重复/幻觉三查，EDL 前置保险丝）
├── pauses_report.md
├── edl.json
