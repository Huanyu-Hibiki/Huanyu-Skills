...
description: 核心 A-roll 粗剪管线。使用 Fun-ASR 或 Whisper 做词级转录，结合终稿和分镜用 FFmpeg 进行保守粗剪，输出 EDL、粗剪预览、转录缓存和粗剪交接文件。触发词：粗剪、转录、剪口播、按文稿剪视频。
argument-hint: "[project-path] [--engine fun-asr|whisper] [--output jianying|filmora|render]"
...
1. 检查虚拟环境、`ffmpeg`、`ffprobe`、Fun-ASR/Whisper。
2. 对每个源文件执行 `ffprobe`，把时长、尺寸、帧率写入 inventory。
3. 读取已有缓存；源文件未变化时禁止重复转录。
4. 使用 Fun-ASR 或 Whisper 输出**词级、verbatim** transcript；不能只生成 phrase/SRT。
...
```bash
# 单个视频：Whisper 词级时间戳 + Fun-ASR 中文校准，结果缓存到 Rough/transcripts/
...
其他入口：`pack_transcripts.py`（打包转录）、`timeline_view.py`（时间线检查）、`grade.py`（调色）、`generate_filmora_project.py`（Filmora 工程）、`funasr_srt.py`（Fun-ASR SRT）。
...
| 触发条件 | 一线修复 | 仍失败兜底 |
|---|---|---|
| 转录失败 / ASR 输出为空 | 检查音轨是否存在（`ffprobe` 看 audio stream）、语言参数是否正确，换引擎（Fun-ASR ↔ Whisper）重跑 | 🔴 该源文件标记 blocked 记入 `missing_materials.md`，继续处理其他源，不中断整期 |
...
| GPU 不可用导致转录极慢 | 改用 Fun-ASR（CPU 可跑）或分段并行 | 告知用户预计耗时，由用户决定等或换机 |
