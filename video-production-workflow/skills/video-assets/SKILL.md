...
5. **技术准入探针（下载完成即测，不信任标称）**：运行 `probe_source.py` 实测最小边分辨率 / 时长 / 关键帧最大间隔——stock 平台的"HD/large"档常见 **640×360 放大假高清**；关键帧间隔 >5s 会让渲染 seek 冻结（探针会给出重编码命令，统一 `-g 30 -keyint_min 30` 重编码后归档）。不达标的素材进 `missing_materials` / 未完成清单或走升级链（换词→换库→AI 生成→才问用户），**不进渲染队列**。
6. **授权偏好**：优先免署名源（Pexels/Pixabay/Mixkit 类）；要求署名的来源（CC BY 系）先向用户确认署名成本再下载，不默默接进清单。
7. 每项第三方素材记录来源网站、页面 URL、下载 URL、标题、作者、许可证、商用许可、署名要求、下载日期和风险。
8. **证据截图存档**：承载关键事实的来源页（数据、引用、授权条款页）逐一截图存 `assets/licenses/evidence/`——只存链接不留证据，源站改版后就无法自证。
9. 许可证不清楚时标记 blocked，不放进消费者目录。
10. 用 FFmpeg 标准化音乐、音效、Stock 视频和用户提供的需要处理的副本。
11. 更新 `assets/licenses/media_asset_manifest.json` 和 `assets/logs/ffmpeg_commands.md`。
12. 输出素材摘要和未解决风险，等待用户确认高风险或付费来源。
...
# 技术准入探针（下载后必跑；不达标不进渲染队列）
uv run --project "<合集根>" python "<合集根>/scripts/video-assets/probe_source.py" \
  "<项目>/assets/incoming/clip1.mp4" "<项目>/assets/incoming/clip2.mp4" --min-side 1080
