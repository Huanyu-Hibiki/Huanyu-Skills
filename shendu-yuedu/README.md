...
组织方式参照 [Karpathy LLM Wiki](https://gist.github.com/karpathy) 模式 + [Google Open Knowledge Format](https://github.com/google/okf)（OKF）约定。所有 skill 通过 `C:/work/Huanyu Hub/Huanyu-Knowledge/personal/阅读笔记/`（合集根目录）共享上下文。**越读越聪明**。

### 三层架构

| 层 | 目录 | 谁写 | 规则 |
|---|---|---|---|
| **Raw 原始层** | `YYYY-MM/<书名>/<章节名>/摘抄.md` 等用户真实阅读笔记 | 人 | **不可变**：AI 只读，提取知识但绝不改写 |
| **Wiki 沉淀层** | 拆解目录 / 问题清单 / 知识卡片 / 实践转化 / Hub 报告 | AI | LLM 全权维护：更新、交叉引用、标矛盾，人只审 |
| **Schema 约定层** | 本 README（阅读域）+ `Huanyu-Knowledge/SKILL.md`（vault 级） | 人×AI 共同演进 | 告诉 LLM 目录职责与工作流约定 |
...
本合集的 skill 安装包只保存协议，不保存运行产物。所有深度阅读输出以 Obsidian Vault 为准：

- 基准目录：`C:/work/Huanyu Hub/Huanyu-Knowledge/personal/阅读笔记/`
- Hub/阅读报告：`C:/work/Huanyu Hub/Huanyu-Knowledge/personal/阅读笔记/_system/用户阅读报告.md`
- sy-practice 产出的可执行 skill：`C:/work/Huanyu Hub/Huanyu-Knowledge/skills/<书名>/<skill-slug>/SKILL.md`
- 跨书技能索引同时写入：阅读报告「已萃取技能索引（跨书累积）」 + `C:/work/Huanyu Hub/Huanyu-Knowledge/skills/INDEX.md` + Obsidian Wiki 索引
- 每本书：`C:/work/Huanyu Hub/Huanyu-Knowledge/personal/阅读笔记/YYYY-MM/书名/`
- 每章笔记：`C:/work/Huanyu Hub/Huanyu-Knowledge/personal/阅读笔记/YYYY-MM/书名/章节名/`
