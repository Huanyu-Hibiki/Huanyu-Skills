...
2. [ ] **识别"解法式描述"**：用户说的常是解法而非需求（"我要一个能
   ...
   真问题后再决定解法是否成立
3. [ ] **3 方案选型**：四项问全后、动笔前，给出至多 3 个候选方案
   （如：A 纯 markdown 单文件 / B markdown+scripts 混合 / C
   references 下沉多文件），每个方案一句适用场景 + 取舍依据（对照
   skill-anatomy §6.3 边界判断表）；**用户选定后才进入阶段二**，
   不替用户默认

### 阶段二：起草（workflow.md §2）

4. [ ] **frontmatter**：name 与目录名一致；description 按 anatomy §5
   公式——第一句做什么（动词 + 具体对象，不用形容词），第二句
   `Use when` + 触发词（用户原话，避开与其他 skill 相撞的通用词，
   anatomy §5.2）。例：`提取 PDF 文本与表格、填写表单。Use when
   用户提到 PDF、表格提取。`
5. [ ] **最小工作流**：只装 80% 场景的步骤清单——编号 + 勾选框，一步
   只说"做什么"，"怎么做"下沉（anatomy §6.1）
6. [ ] **边界与危险操作门槛**：写明不做什么（防误触发）；写 / 删 / 改
   类操作先展示将做什么、确认后执行（anatomy §6.2）
7. [ ] **按需建子目录**：SKILL.md 超 200 行才拆 references/（原地留
   1-2 行指针，下沉规则 anatomy §4.2）；有确定性操作才建 scripts/；
   两者皆无则都不建，不预建空目录（anatomy §1）

### 阶段三：复核与落盘（workflow.md §3）

8. [ ] **先自检再交用户**：对照 structure-checklist.md A-E 逐项过完，
   有问题先修复——不把"description 缺触发词"这类低级问题留给用户
   发现
9. [ ] **呈现草案要点，不贴全文**：name、description、工作流步骤概览、
   建了哪些子目录及为什么——全文是给 Agent 读的，用户要的是决策点
10. [ ] **问三件事**：这些使用场景都覆盖了吗？有缺漏或含糊的地方吗？
    哪部分该更详细、哪部分该更简略？
11. [ ] **确认后才落盘**：展示目标路径与将创建的文件清单，用户点头
    才写文件；用户要改就回到阶段二迭代
